...
import { Bookmark, ChevronRight, EyeOff, Phone, RotateCcw, Search } from 'lucide-react';
...
export function SavedBusinesses({ records, ready, cloud, open, restore, startCalls, hide, dismissing }: { records: LeadRecord[]; ready: boolean; cloud: boolean; open: (key: string) => void; restore: (record: LeadRecord) => Promise<void>; startCalls: () => void; hide: (record: LeadRecord) => Promise<void>; dismissing: Set<string> }) {
  ...
  return <div className="sw-saved-page">
    <div className="sw-result-heading"><div><span className="sw-overline">YOUR LEAD BOOK</span><h1>Saved businesses</h1><p>The contact, the context, and the next conversation.</p></div><button className="sw-primary" onClick={startCalls} disabled={!saved.some(r=>digits(r.card.business.phone).length>=7)}><Phone size={15}/>Open call queue</button></div>
    <div className="sw-book-overview"><span><strong>{saved.length}</strong> saved businesses</span><span><strong>{saved.filter(r=>r.activity?.outcome==='callback').length}</strong> callbacks</span><span><strong>{saved.filter(r=>!r.activity).length}</strong> not called yet</span><small>{cloud?'Synced with Convex':'Saved in this workspace'}</small></div>
