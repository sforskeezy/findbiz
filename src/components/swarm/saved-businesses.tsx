...
import { Bookmark, Check, ChevronRight, EyeOff, FunnelPlus, ScanLine, RotateCcw, Search } from 'lucide-react';
import { displayPhone } from './funnel-ui';
...
export function SavedBusinesses({ records, ready, open, restore, findOpportunities, hide, dismissing, toFunnel, inFunnel }: { records: LeadRecord[]; ready: boolean; open: (key: string) => void; restore: (record: LeadRecord) => Promise<void>; findOpportunities: () => void; hide: (record: LeadRecord) => Promise<void>; dismissing: Set<string>; toFunnel?: (record: LeadRecord) => Promise<void>; inFunnel?: (record: LeadRecord) => boolean }) {
  ...
  return <div className="sw-saved-page">
    ...
    <div className="sw-book-table"><div className="sw-book-columns" aria-hidden="true"><span>Business / contact</span><span>Last conversation</span><span>Reported providers</span><span/></div><AnimatePresence initial={false}>{filtered.map(record=><motion.article initial={{opacity:0,y:4}} animate={dismissing.has(record.card.id)&&!reduce?{opacity:1,x:[0,-4,4,-3,0]}:{opacity:1,x:0,y:0}} exit={reduce?{opacity:0}:{opacity:0,scale:.96,x:18,filter:'blur(5px)'}} transition={{duration:.28}} onDoubleClick={event=>{if(tab==='saved'&&!isContactInteraction(event.target)){intent.cancel();void hide(record);}}} key={record.key} className="sw-book-row">
      ...
      <div className="sw-book-actions">{tab==='hidden'?<button className="sw-secondary" disabled={restoring===record.key} onClick={()=>void undo(record)}><RotateCcw size={14}/>Restore</button>:<>{toFunnel&&<button className={`swx-act ${inFunnel?.(record)?'done':''}`} title={inFunnel?.(record)?'Already in your funnel':'Add to funnel'} aria-label={`Add ${record.card.business.name} to funnel`} disabled={inFunnel?.(record)} onClick={()=>void toFunnel(record)}>{inFunnel?.(record)?<Check size={15}/>:<FunnelPlus size={15}/>}</button>}<button className="sw-secondary" onClick={()=>open(record.key)}>Edit notes<ChevronRight size={14}/></button></>}</div>
