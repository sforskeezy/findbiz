...
export function LiveSidebar({
    ...
    <>
      ...
      <nav aria-label="Workspace" className="space-y-1">
        <button type="button" onClick={() => select(onNewChat)} aria-label="New chat" title={compact ? "New chat" : undefined}
          className={cn("live-sidebar-new flex h-11 items-center gap-2.5 rounded-xl bg-[#272725] text-[13px] font-medium text-white transition hover:bg-[#3b4b31] active:scale-[0.98]", compact ? "w-11 justify-center" : "mb-3 w-full px-3")}>
        ...
        <button type="button" onClick={() => select(onHome)} aria-label="Home" aria-current={atHome ? "page" : undefined} title={compact ? "Home" : undefined}
          className={cn("flex h-11 items-center gap-2.5 rounded-xl text-[13px] transition", compact ? "w-11 justify-center" : "w-full px-3", atHome ? "bg-[#eaeae6] font-medium text-[#343432]" : "text-[#5d6155] hover:bg-[#eceee6]")}>
      ...
      {!compact && <div className="mt-5 flex min-h-0 flex-1 flex-col">
        ...
        <label className="mx-1 mb-4 flex h-10 shrink-0 items-center gap-2 rounded-lg border border-[#e1e5d8] bg-white/70 px-2.5 focus-within:border-[#839664] focus-within:ring-2 focus-within:ring-[#839664]/15">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search chats"
            aria-label="Search chats" className="min-w-0 flex-1 bg-transparent text-[12px] text-[#3e4535] outline-none placeholder:text-[#82827a]" />
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pb-3 scrollbar-none">
          {!visible.length ? <p role="status" className="px-3 py-4 text-[12px] leading-5 text-[#707764]">{search ? "No conversations match your search." : "Start a conversation. You can pick it up here anytime."}</p> : visible.map((group) => (
            <section key={group.label} className="mb-4">
              <h3 className="px-3 pb-1.5 text-[10px] font-medium text-[#82827a]">{group.label}</h3>
              {group.items.map((item) => {
                ...
                return <button key={item.id} type="button" onClick={() => select(() => onOpenSession(item.id))}
                  ...
                  className={cn("group my-0.5 flex min-h-11 w-full items-center gap-2.5 rounded-xl border px-2.5 text-left text-[12px] transition", active ? "border-[#dcdcd5] bg-white font-medium text-[#343432] shadow-[0_2px_5px_#30303008]" : "border-transparent text-[#5d5d57] hover:bg-[#eaeae6]")}>
                  {active && busy ? <WorkingDots size={14} /> : <MessageSquare size={14} strokeWidth={1.6} className={cn("shrink-0", active ? "text-[#666660]" : "text-[#a0a099]")} />}
                  <span className="min-w-0 flex-1 truncate">{item.title}</span>
                  <span className="shrink-0 text-[10px] font-normal tabular-nums text-[#82827a]">{relativeTime(item.updatedAt)}</span>
                </button>;
              })}
            </section>
          ))}
          {!search && total > COLLAPSED_CHAT_COUNT && <button type="button" onClick={() => setShowAll((value) => !value)}
            className="flex h-11 w-full items-center gap-2 rounded-lg px-3 text-[12px] text-[#686862] transition hover:bg-[#eaeae6]">
  ...
  return <>
    <aside aria-label="Sidebar" className={cn("live-sidebar hidden h-full shrink-0 flex-col border-r border-[#e5e5e0] bg-[#f5f5f3] px-3 transition-[width] duration-300 ease-out lg:flex", collapsed ? "w-[68px]" : "w-[272px]")}>
      {content(collapsed)}
    </aside>
    <button type="button" onClick={() => dialogRef.current?.showModal()} aria-label="Open sidebar"
      className="absolute top-[14px] left-3 z-30 flex h-11 w-11 items-center justify-center rounded-xl border border-[#e5e5e0] bg-[#f5f5f3] text-[#555550] lg:hidden">
      <Menu size={20} />
    </button>
    <dialog ref={dialogRef} aria-label="Conversations" onClick={(event) => { if (event.target === event.currentTarget) dialogRef.current?.close(); }}
      className="live-sidebar-drawer fixed inset-y-0 left-0 m-0 h-[100dvh] max-h-none w-[min(320px,88vw)] max-w-none border-0 bg-[#f5f5f3] p-0 text-[#333330] shadow-xl backdrop:bg-[#202020]/30 backdrop:backdrop-blur-sm">
