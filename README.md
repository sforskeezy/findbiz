...
**Funnel** lives under Saved businesses. Add leads manually or drop `.xlsx`, `.csv`, `.tsv`, and `.txt` files into the workspace; imported rows merge with matching leads and keep existing notes and sales stages. Paste or type rough lead details into Quick capture to organize names, numbers, notes, and follow-ups, then review the editable fields. Existing leads have a voice update button that uses Live's microphone and transcription flow; after a pause, it organizes and saves the conversation update. Each lead stores business name, phone and/or account number, contact, information and notes, a follow-up date and time, lead type, and a blue/red/yellow/green sales stage. “Stand” identifies standalone mobile leads; “400 to 750” identifies a speed upgrade. Search, filter, archive and restore, and export the current view as CSV. Previously saved Note documents remain in storage, but the Notes workspace has been replaced by Funnel in navigation.
