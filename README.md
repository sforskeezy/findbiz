...
**Saved businesses** retains complete profiles, research, contact names, editable notes and the latest call outcome in the workspace database. Existing browser IndexedDB records import once without overwriting newer server records. **Never see again** excludes that business location from all batches, maps, Opportunity Finder and exports; Hidden provides a restore action.

**Funnel** lives under Saved businesses in Swarm mode and replaces the old Notes workspace.

- **Stages:** blue = up in the air (haven't spoken yet), yellow = in conversation (50/50 or better), red = close ASAP (next 1–2 days), green = sold. Change a lead's color by clicking the colored square on its row, pressing `1`–`4` while hovering a row, dragging a card between columns in the board view, or picking a stage in the lead panel. Every change has Undo.
- **Import:** drop `.xlsx`, `.csv`, `.tsv`, or `.txt` files anywhere on the page, use Import, or paste rows copied from Excel. Your usual layout `BIZ NAME > PHONE NUMBER OR ACC # OR BOTH > INFORMATION AND NOTES` works, as do labeled columns or `Business: … / Phone: …` blocks. A preview shows the color breakdown and any duplicates before anything is saved; duplicates merge into existing leads without losing notes.
- **Color detection:** Excel row or cell highlights are read from the fill (solid, gradient, RGB, theme + tint, or legacy palette) or the font color, and bucketed by hue, so any shade of red, yellow/orange/gold, green, or blue maps to the right stage. A status/color column (`hot`, `sold`, `50/50`, `r`/`y`/`g`/`b`…), emoji (🔴🟡🟢🔵), section rows like `SOLD` or `🔴 HOT`, and sheet names also work.
- **AI assist:** when `DASHSCOPE_API_KEY` (Qwen) or `GROQ_API_KEY` is set, the same models used by Live help map unfamiliar spreadsheet headers, pull leads out of free-form text files, and tidy quick-capture and voice updates. Every value the model returns must appear in the source text. Without a key, the rule-based parser handles everything.
- **Working a lead:** the lead panel has one-tap stage changes, Call / Copy number / Copy account, inline-editable details, follow-up quick picks (Today, Tomorrow, In 2 days, Next week), and an activity timeline. Type or dictate an update and it's organized, dated, and saved; the stage and follow-up change automatically when the update says so. Use `J`/`K` to move between leads without closing the panel.
- **Shortcuts:** `N` new lead, `/` search, `B` list/board, `↑`/`↓` move, `Enter` open, `1`–`4` set color.
- **From Swarm:** every prospect row, the business profile, saved businesses, and a multi-select action bar have **Add to funnel**, which creates a blue lead with the phone, category, address, rating, and reported providers in its first note. Rows already in the funnel are tagged.
- "Stand" identifies standalone mobile leads; "400 to 750" identifies a speed upgrade. Both are tagged automatically and can be filtered. Export any view as CSV.

**Opportunity Finder** replaces the call queue. It combines active saved Swarms and saved businesses, deduplicates locations, excludes hidden/closed businesses, and ranks by explicit contact-readiness signals. Filter by industry, reported provider, business phone, named contact, and unworked status. Find similar narrows to the same industry across batches. Select a shortlist, copy a call sheet or export CSV with source addresses and coverage dates. FCC availability is never presented as a current subscribed provider. Partial storage reads are identified, not silently treated as complete.
